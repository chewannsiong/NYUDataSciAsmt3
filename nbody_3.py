"""
    ...
    Negligible performance gained through this approach.
...
def update_vs(bodies, dt, dx, dy, dz, body1, body2):
    bodies[body1][1][0] -= dx * compute_b(bodies[body2][2], dt, dx, dy, dz)
    bodies[body1][1][1] -= dy * compute_b(bodies[body2][2], dt, dx, dy, dz)
    bodies[body1][1][2] -= dz * compute_b(bodies[body2][2], dt, dx, dy, dz)
    bodies[body2][1][0] += dx * compute_b(bodies[body1][2], dt, dx, dy, dz)
    bodies[body2][1][1] += dy * compute_b(bodies[body1][2], dt, dx, dy, dz)
    bodies[body2][1][2] += dz * compute_b(bodies[body1][2], dt, dx, dy, dz)
    return bodies

def update_rs(bodies, dt, vx, vy, vz, body):
    bodies[body][0][0] += dt * vx
    bodies[body][0][1] += dt * vy
    bodies[body][0][2] += dt * vz
    return bodies
    ...
    for body1 in bodies.keys():
        for body2 in bodies.keys():
            if (body1 != body2) and not (body2 in seenit):
                ...
                bodies = update_vs(bodies, dt, dx, dy, dz, body1, body2)
                seenit.append(body1)
        
    for body in bodies.keys():
        (r, [vx, vy, vz], m) = bodies[body]
        bodies = update_rs(bodies, dt, vx, vy, vz, body)
